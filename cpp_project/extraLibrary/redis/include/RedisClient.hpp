...
public:
	...
	inline bool WriteLock(void)
	{
		return true;
	}

	inline bool TryReadLock(void) { /* pthread_rwlock_trywrlock() 락이 걸려있으면 EBUSY, 아님 0 */ return true; }
	inline bool TryWriteLock(void) { /* pthread_rwlock_trywrlock() 락이 걸려있으면 EBUSY, 아님 0 */ return true; }

	inline void LeaveReadLock() 
	{
		// InterlockedDecrement(&_lock_flag);
	}

	inline void LeaveWriteLock(void)
	{
		// InterlockedAdd(&_lock_flag, -eLockFlag::WRITE_FLAG);
	}
...
private:
	CSafeLock(const CSafeLock&) = delete;
	CSafeLock& operator=(const CSafeLock&) = delete;
	CSafeLock(CSafeLock&&) = delete;
	CSafeLock& operator=(CSafeLock&&) = delete;
};
#else
class CSafeLock
{
...
public:
	...
	bool IsValid() const { return (0 == m_queIdleConn.size() ? false : true); }
