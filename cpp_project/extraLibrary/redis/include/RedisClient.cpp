...
uint32_t HASH_SLOT(const std::string &strKey)
{
	...
	int32_t nKeyLen = static_cast<int32_t>(strKey.size());
	int32_t nStart, nEnd; /* start-end indexes of { and  } */
...
static inline int FetchSlot(redisReply *pReply, std::vector<SlotRegion> *pvecSlot)
{
    if (pReply->type == REDIS_REPLY_ARRAY)
    {
        ...
        for (size_t i = 0; i < pReply->elements; ++i)
        {
            ...
            slotReg.nStartSlot = (long)pSubReply->element[0]->integer;
            slotReg.nEndSlot = (long)pSubReply->element[1]->integer;
            slotReg.pRedisServ = nullptr;
            slotReg.strHost = pSubReply->element[2]->element[0]->str;
            slotReg.nPort = (long)pSubReply->element[2]->element[1]->integer;
...
{
    InitMemory(static_cast<int32_t>(vecArg.size()) + 1);
...
{
    InitMemory(static_cast<int32_t>(vecArg2.size()) + 2);
    AppendValue(strArg1);
...
{
    InitMemory(static_cast<int32_t>(setArg2.size()) + 2);
    AppendValue(strArg1);
...
void CRedisCommand::SetArgs(const std::vector<std::string> &vecArg1, const std::string &strArg2)
{
    ...
    InitMemory(static_cast<int32_t>(vecArg1.size()) + 2);
...
void CRedisCommand::SetArgs(const std::vector<std::string> &vecArg1, const std::vector<std::string> &vecArg2)
{
    ...
    InitMemory(static_cast<int32_t>(vecArg1.size()) * 2 + 1);
...
{
    InitMemory(static_cast<int32_t>(mapArg.size()) * 2 + 1);
...
{
    InitMemory(static_cast<int32_t>(mapArg2.size()) * 2 + 2);
    AppendValue(strArg1);
...
{
    InitMemory(static_cast<int32_t>(vecArg3.size()) + 3);
    AppendValue(strArg1);
    AppendValue(strArg2);
...
{
    InitMemory(static_cast<int32_t>(vecArg2.size()) * 2 + 2);
    AppendValue(strArg1);
...
bool CRedisClient::Initialize(const std::string &strHost, int nPort, int nTimeout, int nConnNum)
{
    ...
    if (it == mapInfo.end())
        ...
        m_bCluster = (0 == atoi(it->second.c_str()) ? false : true);
