...
#include <vector>
#include <string>
#include "mrpcs_redis_rpc_info.h"
#include "mrpcs_redis_rpc_info.h"

namespace mir3d
{
	class mrpcs_redis_server_info 
	{
	private:
		bool flag_rpc_info = false;
		mrpcs_redis_rpc_info rpc_info;
		mrpcs_redis_rpc_info* p_rpc_info = nullptr;
	public:
		bool has_rpc_info() const
		{
			return flag_rpc_info;
		}
		mrpcs_redis_rpc_info* alloc_rpc_info()
		{
			flag_rpc_info = true;
			return &rpc_info;
		}
		const mrpcs_redis_rpc_info* get_rpc_info() const
		{
		...
		void set_rpc_info(mrpcs_redis_rpc_info* in_rpc_info)
		{
			if (nullptr == p_rpc_info)
			{
	...
	private:
		...
		mrpcs_redis_rpc_info service_info;
		mrpcs_redis_rpc_info* p_service_info = nullptr;
	public:
		bool has_service_info() const
		{
			return flag_service_info;
		}
		mrpcs_redis_rpc_info* alloc_service_info()
		{
			flag_service_info = true;
			return &service_info;
		}
		const mrpcs_redis_rpc_info* get_service_info() const
		{
		...
		void set_service_info(mrpcs_redis_rpc_info* in_service_info)
		{
			if (nullptr == p_service_info)
			{
