...
#include "struct/mps_exchange_context.h"
#include "struct/mps_item.h"
#include "struct/mrpcs_item_stone_pair.h"

namespace mir3d
{
	class mrpc_exchange_item_withdraw_cs : public RpcPacket 	// 거래소-구매한 아이템 받기
	{
	private:
		bool flag_info = false;
		mps_exchange_context info;
		mps_exchange_context* p_info = nullptr;
	public:
		bool has_info() const
		{
			return flag_info;
		}
		mps_exchange_context* alloc_info()
		{
			flag_info = true;
			return &info;
		}
		const mps_exchange_context* get_info() const
		{
			if (true == flag_info)
			{
				if (nullptr == p_info)
		...
		void set_info(mps_exchange_context* in_info)
		{
			if (nullptr == p_info)
			{
				p_info = in_info;
	...
	public:
		mrpc_exchange_item_withdraw_cs(uint8_t tag) : RpcPacket(mpe_rpc_cmd::exchange_item_withdraw_cs, tag)
		{
		}
		mrpc_exchange_item_withdraw_cs() : RpcPacket(mpe_rpc_cmd::exchange_item_withdraw_cs)
		{
		}
	...
	class mrpc_exchange_item_withdraw_sc : public RpcPacket 	// 거래소-구매한 아이템 받기
	{
	public:
		int64_t key{};
	public:
	...
	private:
		...
		mrpcs_item_stone_pair stone_pair;
	...
	public:
		mrpc_exchange_item_withdraw_sc(uint8_t tag) : RpcPacket(mpe_rpc_cmd::exchange_item_withdraw_sc, tag)
		{
		}
		mrpc_exchange_item_withdraw_sc() : RpcPacket(mpe_rpc_cmd::exchange_item_withdraw_sc)
		{
		}
	...
	public:
		template<typename T_BUFFER> void Serialize(T_BUFFER* pBuffer)
		{
			...
			protocol::SerializeType(pBuffer, &pre_acquired);
			protocol::SerializeType(pBuffer, &type);
