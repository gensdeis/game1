...
#include "stdafx.h"
//#include "ShareCoreMinimal.h"
#define _USE_MATH_DEFINES
//#include <memory.h>
//#include <math.h>
#ifndef _SERVER_VERSION
#include "ShareRecast.h"
#include "ShareRecastAlloc.h"
#include "ShareRecastAssert.h"
#else
#include "ShareRecast.h"
#include "ShareRecastAlloc.h"
#include "ShareRecastAssert.h"
...
namespace CSCommon
{
	...
	static float distToTriMesh(const float* p, const float* verts, const int /*nverts*/, const int* tris, const int ntris)
	{
		...
		if (dmin == _MIRM_FLT_MAX) return -1;
		return dmin;
	}
		...
		rcPolyMeshDetail& dmesh)
	{
		...
		for (int i = 0; i < mesh.npolys; ++i)
		{
			...
			if (dmesh.nverts + nverts > vcap)
			{
				...
				float* nerv = (float*)rcAlloc(sizeof(float)*vcap * 3, RC_ALLOC_PERM);
				if (!nerv)
				{
					ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetail: Out of memory 'nerv' (%d).", vcap * 3);
					return false;
				}
				if (dmesh.nverts)
					memcpy(nerv, dmesh.verts, sizeof(float) * 3 * dmesh.nverts);
				rcFree(dmesh.verts);
				dmesh.verts = nerv;
			}
			for (int j = 0; j < nverts; ++j)
