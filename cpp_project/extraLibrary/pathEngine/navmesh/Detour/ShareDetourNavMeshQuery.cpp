...
#include "ShareDetourNavMeshQuery.h"
#include "ShareDetourNode.h"
#include "ShareDetourAssert.h"
#else
#include "ShareDetourNavMeshQuery.h"
#include "ShareDetourNode.h"
#include "ShareDetourAssert.h"
...
#define SHARE_KINDA_SMALL_NUMBER	(1.e-4f)
//#define _MIRM_FLT_MAX          3.402823466e+38F        // max value
typedef signed int	 		int32;

namespace CSCommon
{
	//SHARE_DEFINE_LOG_CATEGORY_STATIC(LogDebugRaycastCrash, All, All);
...
#if SHARE_WITH_FIXED_AREA_ENTERING_COST
		memset(&m_areaFixedCost, 0, sizeof(m_areaFixedCost));
#endif // SHARE_WITH_FIXED_AREA_ENTERING_COST
	}
		...
		const float* referencePt) const
	{
		...
		*nearestRef = 0;
		...
		dtPolyRef nearest = 0;
		float nearestDistanceSqr = _MIRM_FLT_MAX;
		...
		const float* referencePt, float tolerance) const
	{
		...
		*nearestRef = 0;
		...
		dtStatus dtstatus = DT_FAILURE;
		const float toleranceSq = dtSqr(tolerance);
		float referenceLocation[3];
		dtVcopy(referenceLocation, referencePt ? referencePt : center);

		// Find nearest polygon amongst the nearby polygons.
		float bestScoreInTolerance = _MIRM_FLT_MAX;
		float nearestDistanceSqr = _MIRM_FLT_MAX;
		float nearestVertDist = _MIRM_FLT_MAX;
		...
		for (int i = 0; i < polyCount; ++i)
		{
			...
			{
				if ((dSq < nearestDistanceSqr) || (dSq < nearestDistanceSqr + SHARE_KINDA_SMALL_NUMBER && h < nearestVertDist))
				{
		...
		dtQueryResult& result, float* totalCost) const
	{
		...
		while (!m_openList->empty())
		{
			...
			while (i != DT_NULL_LINK)
			{
				...
				// Cost of current link is SHARE_DT_UNWALKABLE_POLY_COST, skip.
				if (((neighbourNode->flags & DT_NODE_OPEN) && total >= neighbourNode->total) ||
					((neighbourNode->flags & DT_NODE_CLOSED) && total >= neighbourNode->total) ||
					(curCost == SHARE_DT_UNWALKABLE_POLY_COST))
				{
					continue;
				}
	...
	dtStatus dtNavMeshQuery::appendPortals(const int startIdx, const int endIdx, const float* endPos, const std::vector<dtPolyRef>& path,
		dtQueryResult& result, const int options) const
	{
		float startPos[3];
		result.getPos(result.size() - 1, startPos);
	...
	dtStatus dtNavMeshQuery::findStraightPath(const float* startPos, const float* endPos,
		const std::vector<dtPolyRef>& path, const int pathSize,
		dtQueryResult& result, const int options) const
	{
		dtAssert(m_nav);
	...
	/// 
	/// If the hit parameter is a very high value (_MIRM_FLT_MAX), then the ray has hit 
	...
	/// If it reaches the end position's xz-coordinates it will indicate _MIRM_FLT_MAX
	...
	{
		dtAssert(m_nav);
		//SHARE_UE_CLOG(m_nav == nullptr, LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast doesn't have valid navmesh!"));
		...
		*t = 0;
		...
		while (curRef)
		{
			...
			const dtPoly* poly = 0;
			{
				...
				//SHARE_UE_CLOG(it >= (unsigned int)m_nav->getMaxTiles(), LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid tile with ref:0x%X (tileIdx:%d, maxTiles:%d) - out of bounds!"), curRef, it, m_nav->getMaxTiles());
				//SHARE_UE_CLOG(m_nav->getTile(it) == nullptr, LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid tile with ref:0x%X (tileIdx:%d, maxTiles:%d) - empty tile!"), curRef, it, m_nav->getMaxTiles());
				//SHARE_UE_CLOG(m_nav->getTile(it)->header == nullptr, LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid tile with ref:0x%X (tileIdx:%d, maxTiles:%d) - missing tile header!"), curRef, it, m_nav->getMaxTiles());
				//SHARE_UE_CLOG(ip >= (unsigned int)m_nav->getTile(it)->header->polyCount, LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid poly with ref:0x%X (polyIdx:%d, maxPolys:%d)!"), curRef, ip, m_nav->getTile(it)->header->polyCount);
			}
			...
			while (i != DT_NULL_LINK)
			{
				...
				const dtPoly* nextPoly = 0;
				{
					...
					//SHARE_UE_CLOG(it >= (unsigned int)m_nav->getMaxTiles(), LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid nei tile with ref:0x%X (tileIdx:%d, maxTiles:%d) - out of bounds!"), link.ref, it, m_nav->getMaxTiles());
					//SHARE_UE_CLOG(m_nav->getTile(it) == nullptr, LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid nei tile with ref:0x%X (tileIdx:%d, maxTiles:%d) - empty tile!"), link.ref, it, m_nav->getMaxTiles());
					//SHARE_UE_CLOG(m_nav->getTile(it)->header == nullptr, LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid nei tile with ref:0x%X (tileIdx:%d, maxTiles:%d) - missing tile header!"), link.ref, it, m_nav->getMaxTiles());
					//SHARE_UE_CLOG(ip >= (unsigned int)m_nav->getTile(it)->header->polyCount, LogDebugRaycastCrash, Fatal, TEXT("dtNavMeshQuery::raycast tried to access invalid nei poly with ref:0x%X (polyIdx:%d, maxPolys:%d)!"), link.ref, ip, m_nav->getTile(it)->header->polyCount);
				}
