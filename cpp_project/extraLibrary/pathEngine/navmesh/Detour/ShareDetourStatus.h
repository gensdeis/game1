...
#ifndef SHARE_DETOURSTATUS_H
#define SHARE_DETOURSTATUS_H
...
namespace CSCommon
{
	...
	{
		return ((status & DT_SUCCESS) != 0)? true : false;
	}

	// Returns true of status is failure.
	inline bool dtStatusFailed(dtStatus status)
	{
		return ((status & DT_FAILURE) != 0)? true : false;
	}

	// Returns true of status is in progress.
	inline bool dtStatusInProgress(dtStatus status)
	{
		return ((status & DT_IN_PROGRESS) != 0)? true : false;
	}

	// Returns true if specific detail is set.
	inline bool dtStatusDetail(dtStatus status, unsigned int detail)
	{
		return ((status & detail) != 0)? true : false;
	}
}
#endif // SHARE_DETOURSTATUS_H
